...
function readFileLineByLine(filePath) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var total, bag, fileStream, rl, _d, rl_1, rl_1_1, line, e_1_1;
...
function isGameValid(line, bag) {
    var gameID = parseInt(line.split(':')[0].split(' ')[1], 10);
    var splitString = line.split(':')[1].split(';').map(function (item) { return item.trim(); });
    var gameStates = [];
    var greatestCount = [0, 0, 0];
    splitString.forEach(function (obj) {
        var tempArray = obj.split(',');
        gameStates.push(tempArray);
    });
    gameStates.forEach(function (state) {
        state.forEach(function (count) {
            var tempArray = count.split(' ');
            var value = 0;
            var color = "";
            if (tempArray[0] == '') {
